...
import { logCycle, requestInAppUpgrade } from "./actions";
 ...
 * row or in this section's header.
 */
export function CalendarSection({ upcoming, past, cycleEnabled }: Props) {
  ...
  return (
    <section className="space-y-4">
      ...
      {requestOpen ? <RequestSessionDialog onClose={() => setRequestOpen(false)} /> : null}
...
function SessionItem({ session: s, muted = false }: { session: Session; muted?: boolean }) {
  ...
  const upgradeRequested = (s.notes ?? "").includes("[client requested in-app upgrade]");
  ...
  function onRequestUpgrade() {
    setMenuOpen(false);
    startTransition(async () => {
      const result = await requestInAppUpgrade({ sessionId: s.id });
      if (!result.ok) {
        alert(result.error);
        return;
      }
    ...
    >
      <div className="min-w-0">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2">
          ...
          {upgradeRequested ? <Badge tone="signal">upgrade requested</Badge> : null}
          {s.zoom_url ? (
            <a
              href={s.zoom_url}
              target="_blank"
          ...
          {menuOpen ? (
            <>
              ...
              <div className="absolute right-0 top-full z-40 mt-1 min-w-[200px] rounded-2xl border border-[color:var(--color-stone-soft)] bg-[color:var(--color-canvas)] p-1 shadow-[0_12px_32px_-8px_rgba(31,30,27,0.25)]">
                {!isInApp && !upgradeRequested ? (
                  <button
                    type="button"
                    className="block w-full rounded-xl px-3 py-2 text-left text-sm hover:bg-[color:var(--color-parchment)]"
                    onClick={onRequestUpgrade}
                  >
                    request in-app upgrade <span className="text-[color:var(--color-stone)]">(+$5)</span>
                  </button>
                ) : null}
                {s.canCancel ? (
                  <button
